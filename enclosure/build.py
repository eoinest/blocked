...
def tunnel(w,d,h,r,x=0,y=0,z=0,flare=0):
    """Rounded X/Z opening extruded along Y; optional outward (+Y) lead-in."""
    return shape('tunnel',w=w,d=d,h=h,r=r,x=x,y=y,z=z,flare=flare)
...
def scad(node, level=0):
    ...
    if k == 'tunnel':
        w,d,h=node['w'],node['d'],node['h']
        flare=node['flare']
        return prefix + (f"translate([0,{-d/2:g},{h/2:g}]) rotate([-90,0,0]) "
                         f"linear_extrude(height={d:g},scale=[{(w+2*flare)/w:g},{(h+2*flare)/h:g}]) "
                         f"offset(r={node['r']:g},$fn=64) square([{w-2*node['r']:g},{h-2*node['r']:g}],center=true);\n")
    if k == 'hex':
...
def write_scad(parts):
    ...
    result += 'part = "assembly"; // [assembly,base,lid,fit-coupon,case-fastener-coupon,board-fastener-coupon,usb-fit-coupon,print-layout]\n'
...
def export_blender(parts):
    ...
    def primitive(n):
        ...
        if k == 'hex':
            ...
            w, d, r = n['w'] / 2, (n['h'] if k=='tunnel' else n['d']) / 2, n['r']
            points = []
        ...
        if k=='tunnel':
            # (local X,Y,Z) -> (world X,-Z,Y) preserves outward face normals.
            fw=(n['w']+2*n['flare'])/n['w'];fh=(n['h']+2*n['flare'])/n['h']
            verts=[(x*fw+n['x'],n['y']+n['d']/2,y*fh+n['z']+n['h']/2) for x,y in points]
            verts += [(x+n['x'],n['y']-n['d']/2,y+n['z']+n['h']/2) for x,y in points]
    ...
    for name in ('fit-coupon', 'case-fastener-coupon','board-fastener-coupon','usb-fit-coupon'):
    ...
    usb=evaluate(difference(tunnel(socket['width'],socket['length'],socket['height'],1.45,y=usb_y,z=board_top),
                             tunnel(8.34,socket['length']+1,2.56,1.2,y=usb_y,z=board_top+.32)))
...
def render_usb_review():
    """Matched USB views with a two-scene editable review file; canonical file stays intact."""
    import bpy
    from mathutils import Vector
    bpy.ops.wm.open_mainfile(filepath=str(ROOT/'blocked.blend'))
    scene=bpy.context.scene
    scene.name='USB — with ESP32'
    prefixes=('PCB_S2_MINI','USB-C socket','Electrical pad','Mounting annulus',
              'ESP32 |','CRYSTAL |','REGULATOR |','RESET |','BOOT |','LED |','Passive ',
              'RESET actuator','BOOT actuator','PCB_NUT_','PCB_SCREW_')
    board_parts=[]
    for obj in scene.objects:
        is_board=obj.name.startswith(prefixes)
        if is_board:board_parts.append(obj)
        visible=is_board or obj.name in ('base','Studio floor') or obj.type in ('CAMERA','LIGHT') or obj.name.startswith('8 x 8')
        obj.hide_render=not visible
        obj.hide_set(not visible)
    cam=scene.camera
    cam.location=(38,85,72)
    cam.rotation_euler=(Vector((0,6,8))-cam.location).to_track_quat('-Z','Y').to_euler()
    cam.data.ortho_scale=78
    scene.render.resolution_x=1200;scene.render.resolution_y=1000
    scene.render.filepath=str(ROOT/'usb-with-board.png')
    for screen in bpy.data.screens:
        for area in screen.areas:
            if area.type=='VIEW_3D':
                area.spaces.active.region_3d.view_perspective='CAMERA'
                area.spaces.active.shading.color_type='MATERIAL'
    # Full-copy scene keeps per-object visibility independent in Blender's scene selector.
    bpy.ops.scene.new(type='FULL_COPY')
    empty=bpy.context.scene;empty.name='USB — enclosure only'
    for obj in empty.objects:
        if obj.name.startswith(prefixes):
            obj.hide_render=True;obj.hide_set(True)
    empty.render.filepath=str(ROOT/'usb-without-board.png')
    bpy.context.window.scene=scene
    bpy.context.preferences.filepaths.save_version=0
    bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'usb-review.blend'))
    bpy.ops.render.render(write_still=True)
...
if __name__ == '__main__':
    if '--usb-review-only' in sys.argv:
        render_usb_review()
        sys.exit(0)
