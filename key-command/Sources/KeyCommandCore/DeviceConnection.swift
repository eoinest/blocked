...
public final class DeviceConnection {
    ...
    public func poll(now: TimeInterval) {
        ...
        while let newline = buffer.firstIndex(of: 10) {
            ...
            let wasIdentified = (session.deviceID != nil)
            if case .press = session.accept(line) { presses += 1 }
            if !wasIdentified && (session.deviceID != nil) { onStatus?("Button connected") }
        }
